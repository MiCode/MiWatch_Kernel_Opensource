...
#define GLINK_LOG_PAGE_CNT	5   //modifi by ROCK  glink test 20230213 original #define GLINK_LOG_PAGE_CNT	2
...
#define ENABLE_PACKET_COUNTER_DEBUG 1
...
#if ENABLE_PACKET_COUNTER_DEBUG
	uint32_t counter;   //Added for pkt counter   Add by ROCK  glink test 20230213 original
#endif
...
#define XFER_STAT_CHANNELS_MAX 10

#define RX_PACKET_SIZES_MAX 6
#define RX_PACKET_SIZES_DELTA 50

#define RX_TO_RX_DONE_TS_BUCKET_SIZE 6
#define RX_TO_RX_DONE_TS_BUCKET_DELTA_MS 1
#define RX_TO_RX_DONE_TS_BUCKET_DELTA_NS (RX_TO_RX_DONE_TS_BUCKET_DELTA_MS * 1000000)


static ssize_t xfer_stat_read(struct kobject *kobj, struct kobj_attribute *attr, char *buf); // forward declaration
static ssize_t xfer_stat_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count); // forward declaration

static struct glink_slatecom *glink_ptr;
static struct kobject *glink_xfer_sysfs_kobj;
static struct kobj_attribute rx_stat_sysfs_attribute =__ATTR(glink_xfer_info, 0660, xfer_stat_read, xfer_stat_store);
char buffer[200];

u32 total_rx_data_count, total_rx_done_count, total_tx_count;
u64 total_rx_data_size, total_tx_data_size;
u32 xfer_duration_start_sec, xfer_duration_end_sec;


struct rx_debug_info
{
	u32 id;
	u32 size;
	u32 timestamp1;
...
struct glink_xfer_stat
{
    u64 rx_data_count_and_ts[2], rx_done_count_and_ts[2], tx_count_and_ts[2], rx_data_size, tx_data_size;
    u32 intent_req_count;

    struct rx_debug_info *rx_debug_info;
    struct rx_done_debug_info *rx_done_debug_info;
    u8 rx_debug_info_index, rx_done_debug_info_index, debug_max_index;

    u32 rx_packet_sizes[RX_PACKET_SIZES_MAX];
	u32 rx_to_rx_done_ts[RX_TO_RX_DONE_TS_BUCKET_SIZE];

	bool enable_xfer_stat;
};
...
struct glink_slatecom_channel {
	...
	struct glink_xfer_stat *xfer_stat;
};

static struct glink_slatecom_channel *xfer_stat_channels[XFER_STAT_CHANNELS_MAX];
static u8 xfer_stat_channels_index;
...
static ssize_t xfer_stat_read(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
        struct glink_slatecom_channel *channel;
        int cid, count;
        int index, packet_size, duration_ms;
        u64 start_ts, end_ts;

        buf[0] = '\0';
        count = 0;

        pr_info("xfer_stat_read: entry");

        if(xfer_duration_start_sec == 0) {
            start_ts = 0;
            end_ts = 0;
        }
        else if(xfer_duration_end_sec == 0) {
            start_ts = xfer_duration_start_sec;
            end_ts = ktime_get_seconds();
        }
        else {
            start_ts = xfer_duration_start_sec;
            end_ts = xfer_duration_end_sec;
        }

        pr_info("%llu %llu %llu %llu", xfer_duration_start_sec, xfer_duration_end_sec, start_ts, end_ts);

        count = sprintf(buf, "\nTotal counts:\nrx# %u\nrx_done %u\nrx_bytes %llu\ntx# %u\ntx_bytes %llu\n\nChannel Stat duration(s) %llu", total_rx_data_count, total_rx_done_count, total_rx_data_size, total_tx_count, total_tx_data_size, (end_ts - start_ts));
        //count += strlcat(buf, buffer, PAGE_SIZE-count-1);

        idr_for_each_entry(&glink_ptr->rcids, channel, cid) {
            if(channel->xfer_stat) {
                sprintf(buffer, "\n\n%s: rx_stat %d, rx_debug %d\n", channel->name, (int)channel->xfer_stat->enable_xfer_stat, (int)channel->xfer_stat->debug_max_index);
                count = strlcat(buf, buffer, PAGE_SIZE-count-1);

                if(channel->xfer_stat->rx_data_count_and_ts[0] > 0 || channel->xfer_stat->tx_count_and_ts[0] > 0) {

                    sprintf(buffer, "count, last ts(nano): rx %llu, %llu | rx_done %llu, %llu | tx %llu, %llu\n", 
                        channel->xfer_stat->rx_data_count_and_ts[0], channel->xfer_stat->rx_data_count_and_ts[1],
                        channel->xfer_stat->rx_done_count_and_ts[0], channel->xfer_stat->rx_done_count_and_ts[1],
                        channel->xfer_stat->tx_count_and_ts[0], channel->xfer_stat->tx_count_and_ts[1]);
                    count = strlcat(buf, buffer, PAGE_SIZE-count-1);
                
                    sprintf(buffer, "xfer bytes: rx %llu | tx %llu\n", 
                        channel->xfer_stat->rx_data_size, channel->xfer_stat->tx_data_size);
                    count = strlcat(buf, buffer, PAGE_SIZE-count-1);


                    sprintf(buffer, "rx pkt sizes (bytes):\n");
                    count = strlcat(buf, buffer, PAGE_SIZE-count-1);

                    packet_size = RX_PACKET_SIZES_DELTA;
                    index = 0;
                    while(index < (RX_PACKET_SIZES_MAX-1))
                    {
                        sprintf(buffer, " [%d-%d]: %d\n", (packet_size - RX_PACKET_SIZES_DELTA), packet_size-1, channel->xfer_stat->rx_packet_sizes[index]);
                        count = strlcat(buf, buffer, PAGE_SIZE-count-1);
        
                        index++;
                        if(index == (RX_PACKET_SIZES_MAX-1)) {
                            sprintf(buffer, " [%d+]: %d\n", packet_size, channel->xfer_stat->rx_packet_sizes[index]);
                            count = strlcat(buf, buffer, PAGE_SIZE-count-1);
                            break;
                        }
                    ...
                    sprintf(buffer, "intent req: %u\n", channel->xfer_stat->intent_req_count);
                    count = strlcat(buf, buffer, PAGE_SIZE-count-1);


                    sprintf(buffer, "rx to rx_done ts(ms):\n");
                    count = strlcat(buf, buffer, PAGE_SIZE-count-1);

                    duration_ms = RX_TO_RX_DONE_TS_BUCKET_DELTA_MS;
                    index = 0;
                    while(index < (RX_TO_RX_DONE_TS_BUCKET_SIZE-1))
                    {
                        sprintf(buffer, " [%d-%d.99]: %u\n", (duration_ms - RX_TO_RX_DONE_TS_BUCKET_DELTA_MS), duration_ms-1, channel->xfer_stat->rx_to_rx_done_ts[index]);
                        count = strlcat(buf, buffer, PAGE_SIZE-count-1);

                        index++;
                        if(index == (RX_TO_RX_DONE_TS_BUCKET_SIZE-1)) {
                            sprintf(buffer, " [%d+]: %u\n", duration_ms, channel->xfer_stat->rx_to_rx_done_ts[index]);
                            count = strlcat(buf, buffer, PAGE_SIZE-count-1);
                            break;
                        }
                    ...
                    sprintf(buffer, "No rx or tx happened on this channel after stats enabled\n", channel->xfer_stat->intent_req_count);
                    count = strlcat(buf, buffer, PAGE_SIZE-count-1);
                }
        ...
        pr_info("xfer_stat_read: exit count %d", (int)count);
        return count;
}

static void clear_channel_xfer_stat(struct glink_slatecom_channel *channel)
{
    int index;

    channel->xfer_stat->rx_data_count_and_ts[0] = 0;
    channel->xfer_stat->rx_data_count_and_ts[1] = 0;

    channel->xfer_stat->rx_done_count_and_ts[0] = 0;
    channel->xfer_stat->rx_done_count_and_ts[1] = 0;

    channel->xfer_stat->tx_count_and_ts[0] = 0;
    channel->xfer_stat->tx_count_and_ts[1] = 0;

    channel->xfer_stat->intent_req_count = 0;

    channel->xfer_stat->rx_data_size = 0;
    channel->xfer_stat->tx_data_size = 0;

    xfer_duration_start_sec = 0;
    xfer_duration_end_sec = 0;

    for(index = 0; index < RX_PACKET_SIZES_MAX; index++)
    {
        channel->xfer_stat->rx_packet_sizes[index] = 0;
    }

    for(index = 0; index < RX_TO_RX_DONE_TS_BUCKET_SIZE; index++) {
       channel->xfer_stat->rx_to_rx_done_ts[index]  = 0;
    }

    total_rx_data_count = 0;
    total_rx_data_size = 0;
    total_rx_done_count = 0;
    total_tx_count = 0;
    total_tx_data_size = 0;

}

static void xfer_stat_config_channel(char* channel_name, bool enable_stat)
{
        struct glink_slatecom_channel *channel;
        int cid;

        pr_info("xfer_stat_config_channel: entry");
        mutex_lock(&glink_ptr->idr_lock);

        if(channel_name) {
            idr_for_each_entry(&glink_ptr->rcids, channel, cid) {
                if(!strncmp(channel->name, channel_name, strlen(channel->name)) && channel->xfer_stat) {
                    channel->xfer_stat->enable_xfer_stat = enable_stat;

                    if(enable_stat) {
                        clear_channel_xfer_stat(channel);
                        xfer_duration_start_sec = ktime_get_seconds();
                    }
                    else if (xfer_duration_end_sec == 0) {
                        xfer_duration_end_sec = ktime_get_seconds();
                    }
                    pr_info("xfer_stat_store: channel %s found", channel_name);
                    goto out;
                }
            }
            pr_err("xfer_stat_store: channel %s not found or stat is not enable for this channel", channel_name);
        }
        else
        {
            idr_for_each_entry(&glink_ptr->rcids, channel, cid) {
                if(channel->xfer_stat) {
                    ...
                    pr_info("xfer_stat_store: run for channel %s", channel->name);
                }
            }
            ...
            if(enable_stat) {
                ...
                if(xfer_duration_end_sec == 0)
                    xfer_duration_end_sec = ktime_get_seconds();
            }
        }
out:
        mutex_unlock(&glink_ptr->idr_lock);
        pr_info("xfer_stat_config_channel: exit");
}

static ssize_t xfer_stat_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
        char *cmd_token, *channel_token;

        pr_info("xfer_stat_store: %s, count %d", buf, (int) count);

        cmd_token = strsep((char**)&buf, " \t");

        if(!cmd_token || cmd_token[0] == '\0') {
            pr_err("xfer_stat_store: invalid command");
            goto out;
        }

        pr_info("xfer_stat_store: %c", cmd_token[0]);

        if(cmd_token[0] == '8') { // disale all channels' stat
            xfer_stat_config_channel(NULL, false);
            goto out;
        }
        else if(cmd_token[0] == '9') { // enable all channels' stat
            xfer_stat_config_channel(NULL, true);
            goto out;
        }


        channel_token = strsep((char**)&buf, " \t");

        if(!channel_token || channel_token[0] == '\0') {
            pr_err("xfer_stat_store: invalid channel");
            goto out;
        }

        pr_info("xfer_stat_store: %s", channel_token);


        if(cmd_token[0] == '0') { // disable channel stat
            xfer_stat_config_channel(channel_token, false);
        }
        else if(cmd_token[0] == '1') { // enable channel stat
            xfer_stat_config_channel(channel_token, true);
        }
        else {
            pr_err("xfer_stat_store: invalid cmd: %c", cmd_token[0]);
        }

out:
        pr_info("xfer_stat_store: %s, count %d, return", buf, (int) count);
        return count;
}
...
static void glink_slatecom_tx_write(struct glink_slatecom *glink,
					void *data, size_t dlen, struct glink_slatecom_channel *channel)
{
	int ret;

#if ENABLE_PACKET_COUNTER_DEBUG   //add by ROCK  glink test 20230213 original
	...
	*(int*)(data + 12) = ++(glink->counter);
	...
	//GLINK_ERR(glink, "%s: ROCT_TEST noError %d PKT COUNTER = %d\n",__func__, ret,glink->counter);
	...
	if (dlen) {
		...
		if (ret < 0) {
            ...
            total_tx_count++;
            total_tx_data_size += dlen;

            if(channel && channel->xfer_stat && channel->xfer_stat->enable_xfer_stat) {
                channel->xfer_stat->tx_count_and_ts[0]++;
                channel->xfer_stat->tx_count_and_ts[1] = ktime_get_ns();
                channel->xfer_stat->tx_data_size += dlen;
            }
        }
	}
}
						...
						size_t dlen, bool wait)
{
	...
	glink_slatecom_tx_write(glink, data, dlen, NULL);
					...
					u32 cid, size_t size)
{
    ...
    if(channel->xfer_stat && channel->xfer_stat->enable_xfer_stat) {
        channel->xfer_stat->intent_req_count++;
    }
	...
	if (intent)
		...
		CH_INFO(channel, "intent alloc failed size:%lu ", size);
				...
				bool wait)
{
	...
	intent->offset += len;
	glink_slatecom_tx_write(glink, &req, sizeof(req), channel);
...
static int glink_slatecom_announce_create(struct rpmsg_device *rpdev)
{
	...
	xfer_stat_val = of_property_read_bool(np, "qcom,glink-xfer-stat");
    of_property_read_u32(np, "qcom,glink-xfer-debug", &xfer_debug_val);

    if(xfer_debug_val)
        xfer_stat_val = true;

	if(xfer_stat_val) {
		channel->xfer_stat = kzalloc(sizeof(struct glink_xfer_stat), GFP_KERNEL);
        if(channel->xfer_stat) {
            channel->xfer_stat->enable_xfer_stat = true;
            channel->xfer_stat->rx_debug_info_index = 0;
            channel->xfer_stat->rx_done_debug_info_index = 0;
            channel->xfer_stat->debug_max_index = xfer_debug_val;

            if(xfer_stat_channels_index < XFER_STAT_CHANNELS_MAX) {
                xfer_stat_channels[xfer_stat_channels_index] = channel;
                xfer_stat_channels_index++;
            }

            if(!glink_xfer_sysfs_kobj) {
                glink_xfer_sysfs_kobj = kobject_create_and_add("glink_xfer_stat", NULL);

                if(glink_xfer_sysfs_kobj)
                    sysfs_err = sysfs_create_file(glink_xfer_sysfs_kobj, &rx_stat_sysfs_attribute.attr);
                else
                    pr_err("glink_xfer_stat kobj create failed");

                if (sysfs_err) {
                    pr_debug("glink_xfer_stat kobj create failed");
                }
                else {
                    if(channel->xfer_stat->debug_max_index) {
                        channel->xfer_stat->rx_debug_info = kzalloc((sizeof(struct rx_debug_info) * channel->xfer_stat->debug_max_index), GFP_KERNEL);
                        if(!channel->xfer_stat->rx_debug_info) {
                            pr_err("kzalloc failed for struct rx_debug_info %s", channel->name);
                        }
                        else {
                            channel->xfer_stat->rx_done_debug_info = kzalloc((sizeof(struct rx_done_debug_info) * channel->xfer_stat->debug_max_index), GFP_KERNEL);
                            if(!channel->xfer_stat->rx_done_debug_info) {
                                pr_err("kzalloc failed for struct rx_done_debug_info for channel %s", channel->name);
                                kfree(channel->xfer_stat->rx_debug_info);
                            }
                        }
                    }
                }
            ...
            if(channel->xfer_stat->enable_xfer_stat) {
                xfer_duration_start_sec = ktime_get_seconds();
                xfer_duration_end_sec = 0;
            }
        }
				   ...
				   struct glink_slatecom_rx_intent *intent)
{
    ...
    int ret_val, index;
    u64 diff_ns, duration_ns;

	cmd.id = reuse ? SLATECOM_CMD_RX_DONE_W_REUSE : SLATECOM_CMD_RX_DONE;
	cmd.lcid = cid;
	cmd.liid = iid;
	ret_val = glink_slatecom_tx(glink, &cmd, sizeof(cmd), true);

    if(!ret_val)
    {
        total_rx_done_count++;

    	if(channel->xfer_stat && channel->xfer_stat->enable_xfer_stat && channel->xfer_stat->rx_data_count_and_ts[0] > 0) {

            channel->xfer_stat->rx_done_count_and_ts[0]++;
            channel->xfer_stat->rx_done_count_and_ts[1] = ktime_get_ns();

            diff_ns = channel->xfer_stat->rx_done_count_and_ts[1] - channel->xfer_stat->rx_data_count_and_ts[1];

            duration_ns = RX_TO_RX_DONE_TS_BUCKET_DELTA_NS;
            index = 0;
            while(index < (RX_TO_RX_DONE_TS_BUCKET_SIZE-1))
            {
                if(diff_ns < duration_ns) {
                    channel->xfer_stat->rx_to_rx_done_ts[index]++;
                    break;
                }
            ...
            if(channel->xfer_stat->rx_done_debug_info) {
                channel->xfer_stat->rx_done_debug_info[channel->xfer_stat->rx_done_debug_info_index].id = intent->id;
                channel->xfer_stat->rx_done_debug_info[channel->xfer_stat->rx_done_debug_info_index].tx_failed = ret_val;
                channel->xfer_stat->rx_done_debug_info[channel->xfer_stat->rx_done_debug_info_index].ts_apps = channel->xfer_stat->rx_done_count_and_ts[1];
                channel->xfer_stat->rx_done_debug_info_index++;
                if (channel->xfer_stat->rx_done_debug_info_index >= channel->xfer_stat->debug_max_index)
                    channel->xfer_stat->rx_done_debug_info_index = 0;
            }
        }
    }
			     ...
			     void *rx_data, size_t avail)
{
	...
	CH_INFO(channel, "chunk_size:%d left_size:%d, liid %u, t1 %u, t2 %u\n", chunk_size, left_size, liid, param3, param4);
...
intent->offset += chunk_size;
	...
	if (!left_size) {
        ...
        total_rx_data_count++;
        total_rx_data_size += intent->offset;

        if(channel->xfer_stat && channel->xfer_stat->enable_xfer_stat) {
            channel->xfer_stat->rx_data_count_and_ts[0]++;
            channel->xfer_stat->rx_data_count_and_ts[1] = ktime_get_ns();
            channel->xfer_stat->rx_data_size += intent->offset;

            packet_size = RX_PACKET_SIZES_DELTA;
            index = 0;
            while(index < (RX_PACKET_SIZES_MAX-1))
            {
                if(intent->offset < packet_size) {
                    channel->xfer_stat->rx_packet_sizes[index]++;
                    break;
                }
            ...
            if(channel->xfer_stat->rx_debug_info) {
                channel->xfer_stat->rx_debug_info[channel->xfer_stat->rx_debug_info_index].id = intent->id;
                channel->xfer_stat->rx_debug_info[channel->xfer_stat->rx_debug_info_index].size = intent->offset;
                channel->xfer_stat->rx_debug_info[channel->xfer_stat->rx_debug_info_index].timestamp1 = param3;
                channel->xfer_stat->rx_debug_info[channel->xfer_stat->rx_debug_info_index].timestamp2 = param4;
                channel->xfer_stat->rx_debug_info[channel->xfer_stat->rx_debug_info_index].ts_apps = channel->xfer_stat->rx_data_count_and_ts[1];
                channel->xfer_stat->rx_debug_info_index++;
                if (channel->xfer_stat->rx_debug_info_index >= channel->xfer_stat->debug_max_index)
                    channel->xfer_stat->rx_debug_info_index = 0;
            }
        }
...
static void glink_slatecom_linkup(struct glink_slatecom *glink)
{
	int ret;
	int rc;
    GLINK_INFO(glink, "  ROCK TEST glink_slatecom_linkup \n");
