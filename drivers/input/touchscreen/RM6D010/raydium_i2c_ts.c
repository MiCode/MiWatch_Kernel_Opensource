...
//	struct timer_list irq_timer;
    ...
    bool palmon;  // for userspace to close palm 
    unsigned long current_time;
    bool i2c_err;
    //struct wake_lock wakelock; // wake lock
};
...
volatile unsigned char palm_report_lock;
unsigned char g_uc_msg = 0;
...
static int raydium_i2c_pda_write_not_disable_pda2(struct i2c_client *client,
        unsigned long addr, unsigned char *w_data, unsigned short length)
{
    int retval;
    unsigned char retry;
...
static int raydium_i2c_mode_control(struct i2c_client *client, unsigned char mode)
{
    ...
    switch (mode)
    {
        ...
        case 2: /* Disable INT */
            raydium_irq_control(ts, DISABLE);
            printk("[touch]Raydium disable INT\n");
            break;
        case 3: /* Enable INT */
            raydium_irq_control(ts, ENABLE);
            printk("[touch]Raydium enable INT\n");
            break;
    }
    return 0;
}
...
static int raydium_fw_upgrade(struct i2c_client * client, unsigned char type, unsigned char isSkipLoad, unsigned char isCheckCRCError)
{
    ...
    printk(KERN_INFO "[touch]VVVVVVVV Type : %d ==> Start VVVVVVVV\n", type);
    ...
    g_uc_msg = 1;
    //#enable INT
    printk(KERN_INFO "[touch]wait flash ready => enable INT\n");
    raydium_irq_control(ts, ENABLE);
    ...
    if (retry == 0 && (g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG)
    {
	printk(KERN_INFO "[touch]no interrupt => wait flash ready\n");
	raydium_irq_control(ts, DISABLE);
	g_uc_raydium_flag = 0;
    }
    ...
    if (retry == 0)
    {
	printk(KERN_ERR "[touch]%s, wait flash ready, retry error!\n", __func__);
	ret = ERROR;
	goto exit_upgrade;
    }
	...
	if (retry == 0 && (g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG)
    {
            printk(KERN_INFO "[touch]no interrupt => wait software reset finish\n");
            raydium_irq_control(ts, DISABLE);
            g_uc_raydium_flag = 0;
    }
	...
	g_uc_msg = 1;
	//#enable INT
	printk(KERN_INFO "[touch]wait fw ready => enable INT\n");
	raydium_irq_control(ts, ENABLE);

	buf[0] = 2;
	ret = raydium_i2c_pda_write_not_disable_pda2(client, 0x20000204, buf, 4);
        if (ret < 0)
        {
            goto exit_upgrade;
        }
	...
	if (retry == 0 && (g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG)
	{
		printk(KERN_INFO "[touch]no interrupt => wait fw ready\n");
		raydium_irq_control(ts, DISABLE);
		g_uc_raydium_flag = 0;
	}
...
static ssize_t raydium_driver_palmon_show(struct device *dev,
...
static ssize_t raydium_driver_palmon_store(struct device *dev,
        struct device_attribute *attr,
        const char *buf, size_t count)
{
    int data = 0;
    ...
    if (err < 0) {
        printk("[touch] palm on store error.\n");
		return err;
    }
    printk("[touch] set palm %d\n", data);
    //ts->palmon = (data > 0) ? true : false;
    if (data == 1)
        ts->palmon = true;
    else if (data == 0)
        ts->palmon = false;
    else
        printk("[touch]set palm wrong input value.\n");
    return count;
}
...
static ssize_t raydium_driver_ver_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	int buf_len = 0;
	sprintf(buf, "[touch]Raydium driver version : 0x%x\n", RAYDIUM_VER);
	buf_len = strlen(buf);
	return buf_len + 1;
}
...
*  example:cat raydium_driver_version
*/
static DEVICE_ATTR(raydium_driver_version, S_IRUGO | S_IWUSR,
	raydium_driver_ver_show,
	NULL);

/**
  * node to close palm for userspace
  */
static DEVICE_ATTR(palmon, S_IRUGO | S_IWUSR,
	raydium_driver_palmon_show,
	raydium_driver_palmon_store);
...
static struct attribute *raydium_attributes[] = {
	...
	&dev_attr_raydium_driver_version.attr,
	&dev_attr_palmon.attr,
    NULL
};
...
static int raydium_report_touch(struct raydium_ts_data *raydium_ts, unsigned char *buf, unsigned char u8_points_amount)
{
    unsigned char i, j, offset, u8_point_id;
    signed short s16_x, s16_y, s16_pressure, s16_wx, s16_wy;
	unsigned char u8_touch_count = 0; /* number of touch points */
    ...
    for (i = 0; i < u8_points_amount; i++)
    {
        ...
        if (j == MAX_TOUCH_NUM)
        {
            for (j = MAX_TOUCH_NUM; j < (MAX_TOUCH_NUM * 2); j++)
            {
                ...
                {
                    printk(KERN_INFO "[touch] PT_%d Down\n", u8_point_id);
    ...
    for (i = 0; i < MAX_TOUCH_NUM; i++)
    {
        ...
        {
	    if (gst_slot_status[i].occupied_pt_id != 0xFF) {
                printk(KERN_INFO "[touch] PT_%d Leave\n",gst_slot_status[i].occupied_pt_id );
	    }
    ...
    for (i = 0; i < MAX_TOUCH_NUM; i++)
    {
        if (gst_slot_status[i].need_update)
        {
            ...
            input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_X, s16_x);
            input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_Y, s16_y);
            input_report_abs(raydium_ts->input_dev, ABS_MT_PRESSURE, s16_pressure);
	...
	input_report_key(raydium_ts->input_dev, BTN_TOOL_FINGER, u8_touch_count > 0);
...
static int raydium_read_touchdata(struct raydium_ts_data *data)
{
    ...
    unsigned char tp_status[MAX_TCH_STATUS_PAKAGE_SIZE];
    int ret = 0;
    unsigned char u8_seq_no = 0, u8_points_amount;

    #ifdef GESTURE_EN
    //display sleep-in mode, touch gesture mode
    if (data->blank == FB_BLANK_POWERDOWN)
    {
		g_uc_gesture_status = RAYDIUM_GESTURE_DISABLE;
		printk(KERN_INFO "[touch]%s:gesture judgement\n", __func__);

        if (data->i2c_err) {
            dev_err(&data->client->dev, "[touch]%s: i2c error, go out\n", __func__);
            goto exit_error;
        }

		//read touch point information
		mutex_lock(&data->lock);
		ret = raydium_i2c_pda2_read(data->client, RAYDIUM_PDA2_GESTURE_RPT_ADDR, buf, 1);
		if (ret < 0)
		{
			dev_err(&data->client->dev, "[touch]%s: failed to read data: %d\n", __func__, ret);
			goto exit_error;
		}
		printk(KERN_INFO "[touch]gesture : %d\n", buf[0]);
        ...
        else if (buf[0] == GESTURE_C)
        {/*
            input_report_key(data->input_dev, KEY_QUICKCALL, 1);
            input_sync(data->input_dev);
            mdelay(1);
            input_report_key(data->input_dev, KEY_QUICKCALL, 0);
            input_sync(data->input_dev);
            // get wake lock to protect KEY_QUICKCALL work
            wake_lock_timeout(&data->wakelock,msecs_to_jiffies(500));
            printk(KERN_INFO "[touch]%s:gesture C\n", __func__);
            */
        }
        else
        {
            //raydium_irq_control(data, ENABLE);
            printk(KERN_INFO "[touch]%s:No gesture\n", __func__);
        }
	...
	else
    {
		...
		raydium_report_touch(data, buf, u8_points_amount);
...
static void raydium_work_handler(struct work_struct *work)
{
    ...
    if (raydium_ts->blank == FB_BLANK_UNBLANK) //when display on can use palm to suspend
    {
		...
		if (!palm_report_lock && raydium_ts->palmon == true)
		{
			printk(KERN_INFO "[touch]send palm event.\n");
			input_report_key(raydium_ts->input_dev, KEY_POWER, true); //press sleep key
			input_sync(raydium_ts->input_dev);
			input_report_key(raydium_ts->input_dev, KEY_POWER, false); //release sleep key
			input_sync(raydium_ts->input_dev);
		}   	
...
static void raydium_ts_do_suspend(struct device *dev)
{
    ...
    //if (wake_lock_active(&ts->wakelock) != 0) {
	...
	if (!cancel_work_sync(&ts->work))
		printk(KERN_INFO "[touch]cancel workqueue err\n");
...
static void raydium_ts_do_resume(struct device *dev)
{
	struct raydium_ts_data *ts = dev_get_drvdata(dev);
    printk(KERN_INFO "[touch]%s, %d.\n", __func__, ts->is_suspend);
    if (ts->is_suspend == 0)
	...
	if (raydium_i2c_pda2_set_page(ts->client, RAYDIUM_PDA2_PAGE_0) < 0)
	{
		dev_err(&ts->client->dev, "[touch]%s: failed to set page\n",
			__func__);
	}
	mutex_unlock(&ts->lock);
	msleep(10);
	...
	if (device_may_wakeup(dev)) {
		...
		if (ts->irq_wake) {
			disable_irq_wake(ts->irq);
			ts->irq_wake = 0;
			printk(KERN_INFO "[touch]disable_irq_wake\n");
		}
	} else {
...
{
    raydium_ts_do_suspend(dev);
    return 0;
}

static int raydium_ts_resume(struct device *dev)
{
    raydium_ts_do_resume(dev);
    return 0;
}
...
{/*
    ...
    if (gpio_is_valid(ts->rst))
    {
        ...
        if (ret < 0)
        {
            dev_err(&ts->client->dev, "[touch] close reset gpio request failed");
        }
        gpio_direction_output(ts->rst, 0);
        msleep(RAYDIUM_RESET_INTERVAL_MSEC);//5ms
        gpio_free(ts->rst);
    } else 
		dev_err(&ts->client->dev, "[touch] close reset gpio invalid");
    mutex_unlock(&ts->lock);
...
/*
static void irq_timer_hander(unsigned long arg)
{
	struct raydium_ts_data *raydium_ts = (struct raydium_ts_data *)arg;
	printk(KERN_INFO "[touch]timer_callback\n");
	palm_report_lock = 0;

	if (del_timer(&raydium_ts->irq_timer))
		printk(KERN_INFO "[touch]The timer is still in use...\n");
	else
		printk(KERN_INFO "[touch]Timer module uninstalling\n");
}
*/
#if defined(RAYDIUM_CONFIG_FB)
...
static int raydium_parse_dt(struct device *dev, struct raydium_ts_platform_data *pdata)
{
    ...
    if (rc)
    {
    	printk("[touch]prase dt coodrds fail.\n");
        return rc;
    }
    ...
    {
    	printk("[touch]prase dt reset_gpio fail.\n");
    ...
    if (pdata->irq_gpio < 0)
    {	
    	printk("[touch]prase dt irq_gpio fail.\n");
    ...
    {
    	printk("[touch]prase dt hard-reset-delay-ms fail, return:%d.\n", rc);
        return rc;
    }
    ...
    {
    	printk("[touch]prase dt soft-reset-delay-ms fail, return:%d.\n", rc);
        return rc;
    }
    ...
    {
    	printk("[touch]prase dtnum-max-touches fail, return:%d.\n", rc);
        return rc;
    }
...
static void raydium_input_set(struct input_dev *input_dev, struct raydium_ts_data *ts)
{
	...
	printk("[touch] set abs prarams x[%d], y[%d]\n", ts->x_max, ts->y_max);
    ...
    input_set_abs_params(input_dev, ABS_MT_PRESSURE, 0, PRESS_MAX, 0, 0);//PRESS_MAX
    input_set_abs_params(input_dev, ABS_MT_TOUCH_MAJOR, 0, PRESS_MAX, 0, 0);//WIDTH_MAX
    input_set_abs_params(input_dev, ABS_MT_TOUCH_MINOR, 0, WIDTH_MAX, 0, 0);//WIDTH_MAX
            ...
            const struct i2c_device_id *id)
{
    ...
    struct input_dev *input_dev;
    int ret = 0;
	printk("[touch]raydium_ts_probe\n");
	if (g_xtc_tp_type == 6 || g_xtc_tp_type == 2 || g_xtc_tp_type == 5 || g_xtc_tp_type == 1)
	{
		pr_info("[touch] raydium tp is already init g_xtc_tp_type:%d\n", g_xtc_tp_type);
		return ret;
	}
    ...
    raydium_ts->is_palm = 0;
	/*
	init_timer(&raydium_ts->irq_timer);
	raydium_ts->irq_timer.function = &irq_timer_hander;
	raydium_ts->irq_timer.expires = jiffies + 25 * HZ;
	raydium_ts->irq_timer.data = (unsigned long)raydium_ts;
	*/
    #endif
    raydium_ts->fw_version = 0;
    raydium_ts->palmon = true;
    raydium_ts->current_time = jiffies;
	palm_report_lock = 1;
    raydium_ts->i2c_err = false;

	device_init_wakeup(&client->dev,1);
    //wake_lock_init(&raydium_ts->wakelock, WAKE_LOCK_SUSPEND, "raydium-touchscreen");
...
*/
    ...
    printk("raydium_ts->pdata->soft_rst_dly:%d",raydium_ts->pdata->soft_rst_dly);
    ...
    //input_set_capability(input_dev, EV_KEY, KEY_QUICKCALL);
    #endif
    ...
    printk(KERN_INFO "[touch]pdata irq : %d \n", raydium_ts->pdata->irq_gpio);//13
    printk(KERN_INFO "[touch]client irq : %d, pdata flags : %d \n", client->irq, pdata->irqflags);//108
    
    ret = request_threaded_irq(gpio_to_irq(raydium_ts->pdata->irq_gpio), NULL, raydium_ts_interrupt,
                    pdata->irqflags | IRQF_TRIGGER_LOW | IRQF_ONESHOT, client->dev.driver->name,
                    raydium_ts);
    if (ret < 0)
    {
...
#endif	
	/*
	printk(KERN_INFO "[touch]Timer module installing\n");
	if (mod_timer(&raydium_ts->irq_timer, jiffies + 30 * HZ))
	{
		printk(KERN_INFO "[touch]Error in mod_timer\n");
		palm_report_lock = 0;
	}
	*/
	g_xtc_tp_type = 3;
...
static int raydium_ts_remove(struct i2c_client *client)
{
    ...
    //wake_lock_destroy(&raydium_ts->wakelock);
