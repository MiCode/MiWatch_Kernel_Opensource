...
static int raydium_i2c_pda2_set_page(struct i2c_client *client,
				     unsigned char page)
{
	int retval = -1;
	unsigned char retry;
...
static int raydium_i2c_pda2_set_page_onetime(struct i2c_client *client, unsigned char page)
{
    int retval = -1;
    unsigned char retry;
    unsigned char buf[RAYDIUM_I2C_PDA2_PAGE_LENGTH];
				  ...
				  unsigned short length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		dev_err(&data->client->dev,
			"[touch]%s: I2C write over retry limit.addr[0x%x]\n",__func__, addr);
		retval = -EIO;
	}
...
{
	dev_info(&ts->client->dev, "[touch]%s, %d, disable depth : %u\n",
		__func__, enable, ts->irq_desc->depth);
	if (enable) {
		if (ts->irq_enabled) {
		...
		while (ts->irq_desc->depth > 0) {
			dev_info(&ts->client->dev, "[touch]irq enable\n");
			ts->irq_enabled = true;
			enable_irq(ts->irq);
		}
	} else {
	...
	if (u8_i2c_mode == PDA2_MODE) {
		...
		*(unsigned long *)rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24)
		...
		ret = raydium_i2c_pda2_read(client,
					    (unsigned char)(RAYDIUM_CHECK_I2C_CMD & MASK_8BIT),
					    rbuffer, 4);

		if (ret < 0)
			goto exit_i2c_error;
	} else {
		ret = raydium_i2c_pda_read(client, RAYDIUM_CHECK_I2C_CMD,
					   rbuffer, 4);
		if (ret < 0)
			goto exit_i2c_error;
	}
...
static int raydium_i2c_mode_control(struct i2c_client *client,
				    unsigned char mode)
{
	struct raydium_ts_data *ts =
		(struct raydium_ts_data *)i2c_get_clientdata(client);
	...
	case 0:	/* Disable INT flag */
		dev_info(&ts->client->dev, "[touch]Raydium INT flag : %d\n",
			 ts->irq_enabled);
		disable_irq(ts->irq);
		ts->irq_enabled = false;
		dev_info(&ts->client->dev, "[touch]Raydium irq disable\n");
		break;
	case 1:	/* Enable INT flag */
		...
		enable_irq(ts->irq);
		ts->irq_enabled = true;
		dev_info(&ts->client->dev, "[touch]Raydium irq enable\n");
		break;
	case 2: /* Disable INT */
		raydium_irq_control(ts, DISABLE);
		dev_info(&ts->client->dev, "[touch]Raydium disable INT\n");
		break;
	case 3: /* Enable INT */
		raydium_irq_control(ts, ENABLE);
		dev_info(&ts->client->dev, "[touch]Raydium enable INT\n");
		break;
	case 4:
		dev_info(&ts->client->dev, "[touch]Raydium INT depth : %d\n",
			 ts->irq_desc->depth);
		break;
#if ENABLE_ESD_CHECK
	case 5: /* Disable ESD timer */
		g_hw_reset_for_esd = 0;
		dev_info(&ts->client->dev, "[touch]Raydium disable ESD timer : %d\n",
			 g_hw_reset_for_esd);
		break;
	case 6: /* Enable ESD timer */
		g_hw_reset_for_esd = 1;
		dev_info(&ts->client->dev, "[touch]Raydium enable ESD timer : %d\n",
			 g_hw_reset_for_esd);
		break;
...
*****************************************************************************/
static unsigned int rc_crc32(const char *buf, unsigned int len,
			     unsigned int crc)
{
	unsigned int i;
...
{
	struct raydium_ts_data *ts = container_of(work, struct raydium_ts_data, esd_work.work);
	if (time_is_after_jiffies(ts->last_valid_interrupt + msecs_to_jiffies(ts->esd_timeout))) {
		dev_err(&ts->client->dev, "[touch] %s %d, reschedule\n", __func__, __LINE__);
		goto reschedule;
	}
...
reschedule:
	schedule_delayed_work(&ts->esd_work, round_jiffies_relative(msecs_to_jiffies(ts->esd_timeout)));
}
#endif
/* upgrade firmware with image file */
static int raydium_fw_upgrade_with_image(struct i2c_client *client,
		unsigned long ul_fw_addr,
		unsigned char type)
{
	int ret = -1;
	...
	case RAYDIUM_BASELINE:
		if (g_receive_fw == 1)
			...
			memcpy(firmware_data, (u8_rad_fw_image + 0xA04), 0x2CC);
		ul_fw_size = 0x2CC;
		break;
	case RAYDIUM_COMP:
		if (g_receive_fw == 1)
			...
			memcpy(firmware_data, (u8_rad_fw_image + 0xCD4), 0x244);
		ul_fw_size = 0x244;
		break;
	case RAYDIUM_PARA:
		if (g_receive_fw == 1)
			...
			memcpy(firmware_data, (u8_rad_fw_image + 0xF1C), 0xE0);
		ul_fw_size = 0xE0;
		break;
	case RAYDIUM_FIRMWARE:
		if (g_receive_fw == 1)
			...
			memcpy(firmware_data, (u8_rad_fw_image + 0x1000), 0x4FFC);
		ul_fw_size = 0x4FFC;
		break;
	case RAYDIUM_BOOTLOADER:
		if (g_receive_fw == 1) {
			...
			memcpy((firmware_data + 0x800), u8_rad_init_image, 0x1FC);
		}
		ul_fw_size = 0x9FC;
		break;
	case RAYDIUM_TEST_FW:
		if (g_receive_fw == 1)
			...
			memcpy((firmware_data + 4), (u8_rad_testfw_image + 0xA04), 0x55FC);
		ul_fw_size = 0x55FC;
		break;
	}

	if (type == RAYDIUM_TEST_FW) {
		ui_checksum = rc_crc32((firmware_data + 4), ul_fw_size,
				       ui_checksum);
		dev_info(&ts->client->dev,
			 "[touch]Raydium ui_checksum : 0x%08X\n", ui_checksum);
	...
	while (ul_write_offset < ul_fw_size) {
		if ((ul_write_offset + MAX_WRITE_PACKET_SIZE) < ul_fw_size)
			...
			us_write_length =
				(unsigned short)(ul_fw_size - ul_write_offset);
...
static int wait_fw_state(struct i2c_client *client, unsigned int u32_addr,
...
static int wait_irq_state(struct i2c_client *client, unsigned int retry_time, unsigned long delay_us)
{
	int ret = SUCCESS;
	unsigned int retry;
	...
	retry = retry_time;
	while (retry != 0 && (g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG) {
		usleep_range(min_delay_us, max_delay_us);
		retry--;
	}

	if ((g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) == RAYDIUM_INTERRUPT_FLAG) {
		retry = retry_time;
		irq_value = 0;
...
static int raydium_do_software_reset(struct i2c_client *client)
{
	...
	buf[0] = 0x01;
	dev_info(&ts->client->dev, "[touch]Raydium SW reset\n");
	...
	while (retry != 0 && (g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]wait software reset finish =>");
		dev_info(&ts->client->dev,
			 " confirm interrupt_flag result %d\n", retry);
	}

	/*#disable INT*/
	if (retry  == 0 && (g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG) {
		dev_info(&ts->client->dev,
			"[touch]no interrupt => disable INT\n");
		raydium_irq_control(ts, DISABLE);
		/*g_uc_raydium_flag = 0;*/
		/*g_uc_raydium_flag &= ~RAYDIUM_INTERRUPT_FLAG;*/
	}
...
static int raydium_wait_ft_int(struct i2c_client *client)
{
	...
	if (retry == 0) {
		dev_info(&ts->client->dev,
			 "[touch]%s, FT Test not ready, retry error!\n",
			 __func__);
		ret = ERROR;
	} else
		raydium_irq_control(ts, ENABLE);
...
static int raydium_load_test_fw(struct i2c_client *client)
{
	...
	if (u8_is_mcu_hold == 0) {
		...
		retry = retry_time;
		while ((retry != 0) &&
		       ((g_uc_raydium_flag & RAYDIUM_INTERRUPT_FLAG) != RAYDIUM_INTERRUPT_FLAG)) {
...
#else
		ret = wait_irq_state(client, 200, 10000);
		if (ret == ERROR)
			goto exit_upgrade;
#endif
	} else {
...
static int raydium_fw_upgrade(struct i2c_client *client,
			      unsigned char type,
			      unsigned char is_skip_load,
			      unsigned char is_check_crc_error)
{
	int ret = 0;
	unsigned char buf[4];
	...
	while (buf[0] != 63 && retry != 0) {
		dev_info(&ts->client->dev,
			 "[touch]confirm in burn mode 0x%x\n", buf[0]);
	...
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, confirm in burn mode 0x%x\n", buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
#else
	/*#confirm in burn mode*/
	if (wait_fw_state(client, 0x50000900, 63, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
	...
	*/
	...
	while (buf[0] != 161 && retry != 0) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]Confirm g_u8_sync_data.ret_data");
		dev_info(&ts->client->dev,
			 "is SET_ADDR_READY %d\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, g_u8_sync_data.ret_data");
		dev_err(&ts->client->dev,
			"is SET_ADDR_READY %d\n", buf[0]);
		ret = -1;
		goto exit_upgrade;
	...
	/*#Wait bootloader check addr and PRAM unlock*/
	/*#Confirm g_u8_sync_data.ret_data at 0x20000214 is SET_ADDR_READY*/
	if (wait_fw_state(client, 0x20000214, 161, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
			"[touch]Error, SET_ADDR_READY\n");
		ret = ERROR;
		goto exit_upgrade;
	}

	/*#Confirm g_u8_sync_data.cmd_type at 0x20000210 is WRT_PRAM_DATA*/
	if (wait_fw_state(client, 0x20000210, 163, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
			"[touch]Error, WRT_PRAM_DATA\n");
		ret = ERROR;
		goto exit_upgrade;
	}
#endif
	...
	if (type == RAYDIUM_INIT) {
		ret = raydium_fw_upgrade_with_image(client, 0x800,
						    RAYDIUM_INIT);
		if (ret < 0)
			goto exit_upgrade;
	}  else if (type == RAYDIUM_PARA) {
		ret = raydium_fw_upgrade_with_image(client, 0xF1C,
						    RAYDIUM_PARA);
		if (ret < 0)
			goto exit_upgrade;
	} else if (type == RAYDIUM_FIRMWARE) {
		...
		if (ret < 0)
			goto exit_upgrade;
		ret = raydium_fw_upgrade_with_image(client, 0x1000,
						    RAYDIUM_FIRMWARE);
		if (ret < 0)
			goto exit_upgrade;

	} else if (type == RAYDIUM_BOOTLOADER) {
		ret = raydium_fw_upgrade_with_image(client, 0x4000,
						    RAYDIUM_BOOTLOADER);
		if (ret < 0)
			goto exit_upgrade;
	}
	...
	while (buf[0] != 165 && retry != 0) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]wait(checksum okay) ACK cmd 0x%x\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, wait(checksum okay) ACK cmd 0x%x\n",
			buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
	...
	while (buf[0] != 165 && retry != 0) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]confirm ACK cmd result 0x%x\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, confirm ACK cmd result 0x%x\n", buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
#else
	 ...
	 * (gu8I2CSyncData.cmd_type=0xa5 at 0x20000210)
	 */
	if (wait_fw_state(client, 0x20000210, 165, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
			"[touch]Error, WRT_CHECKSUM_OK\n");
		ret = ERROR;
		goto exit_upgrade;
	}

	/*#confirm ACK cmd result(gu8I2CSyncData.ret_data=0xa5 at 0x20000214)*/
	if (wait_fw_state(client, 0x20000214, 165, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
	...
	while (buf[0] != 168 && retry != 0) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]wait for unlock key 0x%x\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, wait for unlock key 0x%x\n", buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
#else
	/*#wait for input unlock key*/
	if (wait_fw_state(client, 0x20000210, 168, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
	...
	while (buf[0] != 6 && retry != 0) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]confirm burning_state %d\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, confirm burning_state %d\n", buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
#else
	...
	while (buf[0] != 130 && retry != 0) {
		...
		retry--;
		dev_info(&ts->client->dev,
			 "[touch]wait sw reset finished %d\n", buf[0]);
	}
	if (retry == 0) {
		dev_err(&ts->client->dev,
			"[touch]Error, wait sw reset finished %d\n", buf[0]);
		ret = -1;
		goto exit_upgrade;
	}
#else
	/* wait sw reset finished 0x20000214 = 0x82 */
	if (wait_fw_state(client, 0x20000214, 130, 10000, retry_time) == ERROR) {
		dev_err(&ts->client->dev,
	...
	if (type == RAYDIUM_BASELINE || type == RAYDIUM_INIT ||
	    is_check_crc_error == 1) {
		/*#set test_mode = 1*/
		memset(buf, 0, sizeof(buf));
		buf[0] = 0x01;
		ret = raydium_i2c_pda_write(client, 0x20000218, buf, 4);
		...
		/* confirm gu8I2CSyncData.normal_state = NORMAL_CRC_NOTIFY at
		 * 0x20000208
		 */
		buf[0] = 0;
		retry = retry_time;
		while (buf[0] != 2 && retry != 0) {
			...
			retry--;
			dev_info(&ts->client->dev,
				 "[touch]confirm gu8I2CSyncData.normal_state");
			dev_info(&ts->client->dev,
				 "=NORMAL_CRC_NOTIFY %d\n", buf[0]);
		}
		if (retry == 0) {
			dev_err(&ts->client->dev,
				"[touch]Error, confirm gu8I2CSyncData.normal_state");
			dev_err(&ts->client->dev,
				" =NORMAL_CRC_NOTIFY %d\n", buf[0]);
			ret = -1;
			goto exit_upgrade;
...
#else
		/*#wait crc check finish*/
		if (wait_fw_state(client, 0x20000208, 2, 10000, retry_time) == ERROR) {
			dev_err(&ts->client->dev,
		...
		/*#crc check pass 0x20000214 = 0x81*/
		if (wait_fw_state(client, 0x20000214, 0x81, 10000, retry_time) == ERROR) {
			dev_err(&ts->client->dev,
				"[touch]Error, confirm crc result\n");
			ret = ERROR;
			goto exit_upgrade;
	...
	if (is_check_crc_error) {
		...
		/*#wait reset finish*/
		dev_info(&ts->client->dev, "[touch]Raydium wait bootloader ready\n");
		ret = wait_irq_state(client, 200, 10000);
		if (ret == ERROR)
			goto exit_upgrade;
		...
		dev_info(&ts->client->dev, "[touch]Raydium wait FW ready\n");
		raydium_irq_control(ts, ENABLE);
		ret = raydium_i2c_pda_write(client, 0x20000204, buf, 2);
		if (ret < 0)
			goto exit_upgrade;
...
static int raydium_fw_upgrade_with_bin_file(struct i2c_client *client,
		char *arguments,
		size_t count,
		struct device *dev)
{
	...
	if (crc_buf  == NULL) {
		pr_err("[touch]kzalloc crc_buf failed\n");
		/*kfree(firmware_data);*/
		return -ENOMEM;
	}
	...
	while (uc_fw_amount--) {
		...
		while (ul_write_offset < fw->size) {
			if ((ul_write_offset + MAX_WRITE_PACKET_SIZE)
				...
				us_write_length =
					(unsigned short)(fw->size - ul_write_offset);

			/*write data*/
			ret = raydium_i2c_pda_loadfw(
				      client,
			...
			ret = raydium_i2c_pda_read(client,
					    ...
					    us_write_length);
			if (ret < 0) {
				pr_err("[touch]raydium_i2c_pda_read failed\n");
				goto exit_upgrade;
			}

			for (u16_ii = 0; u16_ii < us_write_length; u16_ii++) {
				if (rbuffer[u16_ii] != fw->data[u16_ii + ul_write_offset]) {
					dev_info(&ts->client->dev,
						"[touch]raydium_fw_upgrade_with_bin_file data NG!!\n");
					dev_info(&ts->client->dev,
						"[touch]rbuffer[%x] : %x!!\n", (u16_ii+ul_write_offset), rbuffer[u16_ii]);
					return -ENOENT;
				}
			}
...
#if 0
/*backup raydium_fw_upgrade_with_bin_file*/
static int raydium_fw_upgrade_with_bin_file(struct i2c_client *client,
		char *arguments,
		size_t count,
		struct device *dev)
...
char gc_test_flag;
static ssize_t raydium_upgrade_firmware_show(struct device *dev,
				 struct device_attribute *attr,
				 char *buf)
{
	int buf_len = 0;
		...
		const char *buf, size_t count)
{
	...
	gc_test_flag = raydium_fw_upgrade_with_bin_file(client, fwname, count, dev);
	mutex_unlock(&ts->lock);
	if (ret < 0)
		return ret;
		...
		const char *buf, size_t count)
{
	...
	if (temp_buf == NULL) {
		dev_info(&ts->client->dev, "[touch]kzalloc temp_buf failed\n");
		return -ENOMEM;
	}

	token = kzalloc(count + 1, GFP_KERNEL);
	if (token == NULL) {
		dev_info(&ts->client->dev, "[touch]kzalloc token failed\n");
		kfree(temp_buf);
		return -ENOMEM;
	}
	...
	if (temp_buf && data_count) {
		...
		mutex_lock(&ts->lock);
		result = raydium_i2c_pda_write(client, g_ul_addr,
					       w_data, g_ui_length);
		mutex_unlock(&ts->lock);
		if (result < 0)
			ret = result;
		...
		const char *buf, size_t count)
{
	...
	token = kzalloc(count + 1, GFP_KERNEL);
	if (token  == NULL) {
		dev_err(&ts->client->dev, "[touch]kzalloc token failed\n");
		kfree(temp_buf);
		return -ENOMEM;
	}
		...
		char *buf)
{
	...
	while (ui_read_offset < g_ui_raw_data_length) {
		if ((ui_read_offset + MAX_READ_PACKET_SIZE) <
			...
			us_read_length =
			(unsigned short)(g_ui_raw_data_length - ui_read_offset);

		ui_read_target_addr =
			RAYDIUM_READ_FT_DATA_CMD + ui_read_offset;
				 ...
				 char *buf)
{
	...
	sprintf(buf, "%d", g_uc_raydium_flag);
	dev_info(&ts->client->dev, "[touch]Raydium int flag = %s!!\n", buf);
					...
					char *buf)
{
	...
	struct raydium_ts_data *ts =
		(struct raydium_ts_data *)i2c_get_clientdata(client);
	unsigned char tp_status[MAX_TCH_STATUS_PACKET_SIZE];
	unsigned char tp_buf[MAX_REPORT_PACKET_SIZE];
				       ...
				       const char *buf, size_t count)
{
	...
	*(unsigned long *)rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24) |
	...
	ret = raydium_i2c_pda2_write(client, (unsigned char)(ui_read_target_addr & MASK_8BIT), rbuffer, 4);
	if (ret < 0)
		goto exit_error;
	...
	*(unsigned long *)rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24) |
	...
	ret = raydium_i2c_pda2_read(client, (unsigned char)(ui_read_target_addr & MASK_8BIT), rbuffer, 4);
	if (ret < 0)
		goto exit_error;
				      ...
				      const char *buf, size_t count)
{
	...
	dev_info(&ts->client->dev, "[touch]Raydium int flag store = %d!!\n",
		...
		const char *buf, size_t count)
{
	...
	dev_info(&ts->client->dev, "[touch]Raydium %s!!\n", __func__);
	...
	dev_info(&ts->client->dev,
		 "[touch]Raydium selftest flag store = %d!!\n",
					...
					const char *buf, size_t count)
{
	...
	case 0: /* Disable Touch lock */
		...
		dev_info(&ts->client->dev,
			 "[touch]Raydium %s disable touch lock!!\n", __func__);
#if ENABLE_ESD_CHECK
		g_hw_reset_for_esd = 1;
#endif
		ts->is_sleep = 0;
	...
	case 1: /* Enable Touch lock */
		...
		if (ret < 0)
			goto exit_i2c_error;
		dev_info(&ts->client->dev,
			 "[touch]Raydium %s enable touch lock!!\n", __func__);
		ts->is_sleep = 1;
		break;
	}
		...
		char *buf)
{
	...
	if (ret < 0)
		goto exit_i2c_error;
	ret = raydium_i2c_pda2_read(client, RAYDIUM_PDA2_FW_VERSION_ADDR,
				    rbuffer, 4);
	if (ret < 0)
		goto exit_i2c_error;
	...
	fw_version = (rbuffer[0] << 24) | (rbuffer[1] << 16) | (rbuffer[2] << 8)
		     | rbuffer[3];
	dev_err(&ts->client->dev,
	...
	if (g_receive_fw == 1) {
		...
		image_version = (u8_rad_fw_image[0xF21] << 24) |
			(u8_rad_fw_image[0xF22] << 16) |
			(u8_rad_fw_image[0xF23] << 8) |
			u8_rad_fw_image[0xF24];
	}
	dev_info(&ts->client->dev,
		...
		char *buf)
{
	...
	if (ret < 0)
		goto exit_i2c_error;
	ret = raydium_i2c_pda2_read(client, RAYDIUM_PDA2_PANEL_VERSION_ADDR,
				    rbuffer, 8);
	if (ret < 0)
		goto exit_i2c_error;
...
static ssize_t fw_check_show(struct device *dev,
			     struct device_attribute *attr,
			     char *buf)
{
	int count = 0;
...
static unsigned char i2c_burst_read(struct i2c_client *client,
				    int int_addr,
				    unsigned short u16ReadLen,
...
static unsigned char i2c_burst_write(struct i2c_client *client,
				     int int_addr,
	...
	while (i16_time_out > 0 && u8_is_pass != 1) {
		...
		while (u32_index < u32_test_length) {
			if (i2c_burst_write(client, addr + u32_index,
					    u32_i2c_writelen, u8_write_data)
			    == 0) {
				dev_err(&ts->client->dev,
		...
		while (u32_readlen < u32_test_length) {
			...
			if (u32_test_length - u32_readlen < u32_i2c_readlen)
				u32_i2c_readlen = u32_test_length - u32_readlen;
		}
		...
		const char *buf, size_t count)
{
	...
	if ((buf[0] == '#') && (count == 2)) {/*scheck FW type*/
		if (buf[1] == '1')
			fw_type = 1;
		else if (buf[1] == '2')
			fw_type = 2;
		...
		dev_info(&ts->client->dev, "[touch] fw_type=%d, fwlength=%d\n", fw_type, fwlength);
		if (fw_type == 1) {
			rad_fw_length = fwlength;
			g_rad_fw_image = kmalloc(rad_fw_length*sizeof(char), GFP_KERNEL);
			if (g_rad_fw_image  == NULL) {
				dev_err(&ts->client->dev, "[touch] g_rad_fw_image kzalloc failed\n");
				return -ENOMEM;
			}
		} else if (fw_type == 2) {
			rad_init_length = fwlength;
			g_rad_init_image = kmalloc(rad_init_length*sizeof(char), GFP_KERNEL);
			if (g_rad_init_image  == NULL) {
				dev_err(&ts->client->dev, "[touch] g_rad_init_image kzalloc failed\n");
				return -ENOMEM;
			}
		} else if (fw_type == 3) {
			rad_boot_length = fwlength;
			g_rad_boot_image = kmalloc(rad_boot_length*sizeof(char), GFP_KERNEL);
			if (g_rad_boot_image  == NULL) {
				dev_err(&ts->client->dev, "[touch] g_rad_boot_image kzalloc failed\n");
				return -ENOMEM;
			}
		} else if (fw_type == 4) {
			rad_testfw_length = fwlength;
			g_rad_testfw_image = kmalloc(rad_testfw_length*sizeof(char), GFP_KERNEL);
			if (g_rad_testfw_image  == NULL) {
				dev_err(&ts->client->dev, "[touch] g_rad_testfw_image kzalloc failed\n");
				return -ENOMEM;
			}
		}
		...
		dev_info(&ts->client->dev, "[touch] rad_fw_length=%d, rad_init_length=%d, rad_boot_length=%d, rad_testfw_length=%d\n", rad_fw_length, rad_init_length, rad_boot_length, rad_testfw_length);
		if (rad_fw_length != 0) {
		...
		dev_info(&ts->client->dev, "[touch] other case \n");
...
static DEVICE_ATTR(raydium_reset_control, S_IRUGO | S_IWUSR,
		   NULL,
		   raydium_reset_control_store);

static DEVICE_ATTR(raydium_receive_fw_control, S_IRUGO | S_IWUSR,
		   NULL,
...
static int raydium_create_sysfs(struct i2c_client *client)
{
	...
	if (ret) {
		...
		dev_info(&client->dev,
			 "[touch]create raydium sysfs attr_group succesful\n");
	}
	return ret;
}
...
#ifdef FILTER_POINTS
int Is_meet_fixed_pointer_filter(struct raydium_ts_data *raydium_ts,
				int index,
				int diff_x,
				int diff_y)
{
	if (abs(raydium_ts->x_pos[index] - raydium_ts->last_x_pos[index]) >  diff_x)
		return 0;
	if (abs(raydium_ts->y_pos[index] - raydium_ts->last_y_pos[index]) >  diff_y)
		return 0;
...
static int raydium_touch_report(struct raydium_ts_data *raydium_ts,
				    unsigned char *buf,
				    unsigned char u8_points_amount)
{
	unsigned char i, j, offset, u8_point_id;
	signed short s16_wx, s16_wy;
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		if (gst_slot_status[i].need_update) {
			...
			raydium_ts->x_pos[i] = buf[POS_X_L + offset] | buf[POS_X_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			raydium_ts->y_pos[i] = buf[POS_Y_L + offset] | buf[POS_Y_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			raydium_ts->pressure = buf[POS_PRESSURE_L + offset] | buf[POS_PRESSURE_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			...
			/*printk(KERN_INFO "(x[%d],y[%d])=(%d %d) pressure=%d s16_wx=%d s16_wy=%d offset=%d\n", i,i,raydium_ts->x_pos[i],raydium_ts->y_pos[i],raydium_ts->pressure,s16_wx,s16_wy,offset);*/
			if (!Is_meet_fixed_pointer_filter(raydium_ts, i, DELTA_X, DELTA_Y)) {
				input_mt_slot(raydium_ts->input_dev, i);
				input_mt_report_slot_state(raydium_ts->input_dev, MT_TOOL_FINGER, true);
				__set_bit(i, ids);
				input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_X, raydium_ts->x_pos[i]);
				input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_Y, raydium_ts->y_pos[i]);
				input_report_abs(raydium_ts->input_dev, ABS_MT_PRESSURE, raydium_ts->pressure);
				input_report_abs(raydium_ts->input_dev, ABS_MT_TOUCH_MAJOR, max(s16_wx, s16_wy));
				input_report_abs(raydium_ts->input_dev, ABS_MT_TOUCH_MINOR, min(s16_wx, s16_wy));
				input_report_key(raydium_ts->input_dev, BTN_TOUCH, 1);
				input_report_key(raydium_ts->input_dev, BTN_TOOL_FINGER, 1);
				input_sync(raydium_ts->input_dev);
				/*printk(KERN_INFO "Report point (x[%d],y[%d])=(%d %d) last(x[%d],y[%d])=(%d %d) pressure=%d\n",i,i,raydium_ts->x_pos[i],raydium_ts->y_pos[i],i,i,raydium_ts->last_x_pos[i],raydium_ts->last_y_pos[i],raydium_ts->pressure);*/
				raydium_ts->last_x_pos[i] = raydium_ts->x_pos[i];
				raydium_ts->last_y_pos[i] = raydium_ts->y_pos[i];
			} else {
				raydium_ts->x_pos[i] = raydium_ts->last_x_pos[i];
				raydium_ts->y_pos[i] = raydium_ts->last_y_pos[i];
				/*printk(KERN_INFO "No report point (x[%d],y[%d])=(%d %d) last(x[%d],y[%d])=(%d %d) pressure=%d\n",i,i,raydium_ts->x_pos[i],raydium_ts->y_pos[i],i,i,raydium_ts->last_x_pos[i],raydium_ts->last_y_pos[i],raydium_ts->pressure);*/
			}
		}
	}

	if ((gst_slot_status[0].need_update == 0) && (gst_slot_status[1].need_update == 0)) {
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if (test_bit(i, ids))
				continue;
			input_mt_slot(raydium_ts->input_dev, i);
		...
		input_report_key(raydium_ts->input_dev, BTN_TOUCH, 0);
		input_report_key(raydium_ts->input_dev, BTN_TOOL_FINGER, 0);
		input_sync(raydium_ts->input_dev);
	}
#else
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		if (gst_slot_status[i].need_update) {
			offset = gst_slot_status[i].pt_report_offset * LENGTH_PT;
			raydium_ts->x_pos = buf[POS_X_L + offset] | buf[POS_X_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			raydium_ts->y_pos = buf[POS_Y_L + offset] | buf[POS_Y_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			raydium_ts->pressure = buf[POS_PRESSURE_L + offset] | buf[POS_PRESSURE_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			s16_wx = buf[POS_WX_L + offset] | buf[POS_WX_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			s16_wy = buf[POS_WY_L + offset] | buf[POS_WY_H + offset] << SHORT_HIGH_BYTE_SHIFT;
			/*printk(KERN_INFO "(x[%d],y[%d])=(%d %d) pressure=%d s16_wx=%d s16_wy=%d offset=%d\n", i,i,raydium_ts->x_pos[i],raydium_ts->y_pos[i],raydium_ts->pressure,s16_wx,s16_wy,offset);*/
			...
			input_mt_report_slot_state(raydium_ts->input_dev, MT_TOOL_FINGER, true);
			__set_bit(i, ids);
			input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_X, raydium_ts->x_pos);
			input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_Y, raydium_ts->y_pos);
			input_report_abs(raydium_ts->input_dev, ABS_MT_PRESSURE, raydium_ts->pressure);
			input_report_abs(raydium_ts->input_dev, ABS_MT_TOUCH_MAJOR, max(s16_wx, s16_wy));
			input_report_abs(raydium_ts->input_dev, ABS_MT_TOUCH_MINOR, min(s16_wx, s16_wy));
			input_report_key(raydium_ts->input_dev, BTN_TOUCH, 1);
...
static int raydium_read_touchdata(struct raydium_ts_data *data, unsigned char *tp_status,  unsigned char *buf)
{
	int ret = 0;
	unsigned char u8_points_amount;
	...
	memset(buf, 0, MAX_REPORT_PACKET_SIZE);
	memset(tp_status, 0, MAX_TCH_STATUS_PACKET_SIZE);
	/*read touch point information*/
	ret = raydium_i2c_pda2_read(data->client, RAYDIUM_PDA2_TCH_RPT_STATUS_ADDR, tp_status, MAX_TCH_STATUS_PACKET_SIZE);
	if (ret < 0) {
	...
	if (u8_seq_no == tp_status[POS_SEQ]) {
		dev_err(&data->client->dev, "%s -> report not updated.\n", __func__);
		goto exit_error;
	}
...
static void raydium_work_handler(struct work_struct *work)
{
	...
	int ret = 0;
	unsigned char tp_status[MAX_TCH_STATUS_PACKET_SIZE];
	unsigned char buf[MAX_REPORT_PACKET_SIZE];
	...
	if (raydium_ts->blank == FB_BLANK_UNBLANK) {
		...
		} else if ((tp_status[POS_GESTURE_STATUS] == RAYDIUM_PALM_MODE_DISABLE) && (raydium_ts->is_palm == 1)) {
			dev_info(&raydium_ts->client->dev,
				"[touch]leave palm mode.\n");
	...
	} else if (raydium_ts->blank == FB_BLANK_VSYNC_SUSPEND || raydium_ts->blank == FB_BLANK_POWERDOWN) {
		/*need check small area*/
		input_mt_slot(raydium_ts->input_dev, 0);
		input_mt_report_slot_state(raydium_ts->input_dev, MT_TOOL_FINGER, 1);
		input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_X, 100);
		input_report_abs(raydium_ts->input_dev, ABS_MT_POSITION_Y, 100);
		input_sync(raydium_ts->input_dev);
		mdelay(10);
		input_mt_slot(raydium_ts->input_dev, 0);
		input_mt_report_slot_state(raydium_ts->input_dev, MT_TOOL_FINGER, 0);
		input_mt_report_pointer_emulation(raydium_ts->input_dev, false);
		input_sync(raydium_ts->input_dev);
		g_uc_gesture_status = RAYDIUM_GESTURE_DISABLE;
		dev_info(&raydium_ts->client->dev, "[touch]display wake up\n");
		/*goto exit;*/
	}
#endif
...
static int raydium_read_touchdata(struct raydium_ts_data *data)
{
	...
	if (data->blank == FB_BLANK_VSYNC_SUSPEND ||
		...
		input_sync(data->input_dev);
		g_uc_gesture_status = RAYDIUM_GESTURE_DISABLE;
		dev_info(&data->client->dev, "[touch]display wake up\n");
		return 0;
	} else {
#endif
		mutex_lock(&data->lock);
		...
		if (u8_points_amount > MAX_TOUCH_NUM) {
			mutex_unlock(&data->lock);
			dev_err(&data->client->dev,
			"%s -> u8_points_amount > MAX_TOUCH_NUM.\n", __func__);
			return 0;
		}
		...
		u8_seq_no = tp_status[POS_SEQ];
		tp_status[POS_SEQ] = 0;
		ret = raydium_i2c_pda2_write(data->client,
					RAYDIUM_PDA2_TCH_RPT_STATUS_ADDR,
					tp_status,
					1);
		if (ret < 0) {
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if (gst_slot_status[i].need_update) {
				...
				s16_x = buf[POS_X_L + offset] |
					buf[POS_X_H + offset] <<
					SHORT_HIGH_BYTE_SHIFT;

				s16_y = buf[POS_Y_L + offset] |
					buf[POS_Y_H + offset] <<
					SHORT_HIGH_BYTE_SHIFT;

				s16_pressure = buf[POS_PRESSURE_L + offset] |
					buf[POS_PRESSURE_H + offset] <<
					SHORT_HIGH_BYTE_SHIFT;

				s16_wx = buf[POS_WX_L + offset] |
					 (buf[POS_WX_H + offset] <<
					 SHORT_HIGH_BYTE_SHIFT);

				s16_wy = buf[POS_WY_L + offset] |
					 (buf[POS_WY_H + offset] <<
					 SHORT_HIGH_BYTE_SHIFT);
...
printk("Raydium Slot status #: %d %d ; %d %d ; %d %d ; %d %d\n",
	gst_slot_status[0].need_update,
	gst_slot_status[0].occupied_pt_id,
	gst_slot_status[1].need_update,
	gst_slot_status[1].occupied_pt_id,
	gst_slot_status[2].need_update,
	gst_slot_status[2].occupied_pt_id,
	gst_slot_status[3].need_update,
	gst_slot_status[3].occupied_pt_id);
*/
#ifdef GESTURE_EN
	}
#endif
...
static irqreturn_t raydium_ts_interrupt(int irq, void *dev_id)
{
	...
	/*if (g_uc_raydium_selftest_flag == 1) {
		...
		dev_info(&raydium_ts->client->dev,
			"[touch]g_uc_raydium_selftest_flag = 1\n");
	...
	if ((g_uc_raydium_flag & RAYDIUM_ENGINEER_MODE) != 0) {
		...
		if (!work_pending(&raydium_ts->work)) {
			/* Clear interrupts*/
		    	mutex_lock(&raydium_ts->lock);
		    	if (raydium_i2c_pda2_set_page_onetime(raydium_ts->client, RAYDIUM_PDA2_PAGE_0) < 0) {
		          	dev_err(&raydium_ts->client->dev, "[touch]%s: failed to set page ontime\n",__func__);
		    	}
		   	mutex_unlock(&raydium_ts->lock);
				...
				if (raydium_ts->blank == FB_BLANK_POWERDOWN) {
					dev_info(&raydium_ts->client->dev,
					 "[touch]%s, disable depth : %u\n",
					 __func__,
					 raydium_ts->irq_desc->depth);

					disable_irq_nosync(raydium_ts->irq);
					raydium_ts->irq_enabled = false;
					dev_info(&raydium_ts->client->dev,
				     "[touch]FB_BLANK_POWERDOWN disable irq\n");
				}
			}
			} else { /*work pending*/
				/* Clear interrupts*/
				mutex_lock(&raydium_ts->lock);
				if (raydium_i2c_pda2_set_page(raydium_ts->client,
						     RAYDIUM_PDA2_PAGE_0) < 0) {
					disable_irq_nosync(raydium_ts->irq);
					raydium_ts->irq_enabled = false;
					dev_err(&raydium_ts->client->dev,
...
#if ENABLE_ESD_CHECK
	raydium_ts->last_valid_interrupt = jiffies;
#endif
	return IRQ_HANDLED;
}
...
static int raydium_check_i2c_ready(struct raydium_ts_data *raydium_ts)
{
	...
	if (u8_i2c_mode == PDA2_MODE) {
		...
		*(unsigned long *)temp_buf =
		...
		ret = raydium_i2c_pda_read(raydium_ts->client,
					   RAYDIUM_CHECK_I2C_CMD, temp_buf,
					   4);
		if (ret < 0)
			goto exit_error;
	...
	dev_err(&raydium_ts->client->dev,
		"[touch]Raydium Touch check I2C:0x%02X%02X\n",
		temp_buf[3],
		temp_buf[2]);
...
static int raydium_fw_update_check(struct raydium_ts_data *raydium_ts)
{
	...
	ret = raydium_i2c_pda2_read(raydium_ts->client,
	...
	if (fw_version != image_version) {
		dev_err(&raydium_ts->client->dev, "[touch]FW need update.\n");
		raydium_irq_control(raydium_ts, DISABLE);
		if ((g_uc_raydium_flag & RAYDIUM_ENGINEER_MODE) == 0) {
			g_uc_raydium_flag |= RAYDIUM_ENGINEER_MODE;
			u8_mode_change = 1;
		...
		dev_err(&raydium_ts->client->dev,
			"[touch]FW is the latest version.\n");
...
static bool raydium_esd_check(struct i2c_client *client)
{
	...
	if (u8_i2c_mode == PDA2_MODE) {
		...
		if (ret < 0)
			goto exit_i2c_error;
		*(unsigned long *)rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24) | ((RAYDIUM_CHECK_I2C_CMD & (~MASK_8BIT)) >> 8);
		ret = raydium_i2c_pda2_write(client, RAYDIUM_PDA2_PDA_CFG_ADDR, rbuffer, 4);
		if (ret < 0)
			goto exit_i2c_error;
		ret = raydium_i2c_pda2_set_page(client,	RAYDIUM_PDA2_ENABLE_PDA);
		...
		dev_info(&ts->client->dev, "[touch] %s %d\n", __func__, __LINE__);
	...
	if (((rbuffer[3] != 0xf3) || (rbuffer[2] != 0x1)) && (g_hw_reset_for_esd == 1)) { /*define touch FW version*/
		dev_info(&ts->client->dev, "[touch] Wrong touch verion, rbuffer[3]=%x, buffer[2]=%x, g_hw_reset_for_esd=%d, need to reset touch, ts->irq_enabled=%d\n", rbuffer[3], rbuffer[2], g_hw_reset_for_esd, ts->irq_enabled);
	...
	return ret;
exit_i2c_error:
	dev_info(&ts->client->dev, "[touch] %s %d, exit_i2c_error", __func__, __LINE__);
	return ret;
}
#endif
...
static void raydium_ts_do_suspend(struct raydium_ts_data *ts)
{
	...
	if (ts->is_suspend == 1) {
		dev_info(&ts->client->dev, "[touch]Already in suspend state\n");
		return;
	}
	...
	dev_info(&ts->client->dev, "[touch]%s.\n", __func__);
	...
	if (device_may_wakeup(&ts->client->dev)) {
		dev_info(&ts->client->dev, "[touch]Device may wakeup\n");
		if (!enable_irq_wake(ts->irq))
			ts->irq_wake = 1;

	} else
		dev_info(&ts->client->dev, "[touch]Device not wakeup\n");
	raydium_irq_control(ts, ENABLE);
#endif
...
static void raydium_ts_do_resume(struct raydium_ts_data *ts)
{
	...
	if (ts->is_suspend == 0) {
		dev_info(&ts->client->dev, "[touch]Already in resume state\n");
		return;
	}
	...
	if (ts->is_close != 1) {
		dev_info(&ts->client->dev, "[touch]Already in close state\n");
		raydium_irq_control(ts, ENABLE);
	}
...
/*
	...
	dev_info(&ts->client->dev, "[touch]ts->blank:%x\n", ts->blank);
	...
	if (ts->is_sleep == 1) {
		...
		raydium_irq_control(ts, ENABLE);
		ts->is_sleep = 0;
		dev_info(&ts->client->dev, "[touch]disable touch lock.\n");
	}
#if ENABLE_ESD_CHECK
	if (ts->esd_timeout) {
...
/*
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "[touch]ret:%d\n", ret);
		goto exit_i2c_error;
	}

	wbuffer[0] = RAYDIUM_HOST_CMD_PWR_SLEEP;
	ret = raydium_i2c_pda2_write(ts->client,
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "[touch]Disable M0 NG ret:%d\n", ret);
		goto exit_i2c_error;
	}
	...
	if (ret)
		dev_err(&ts->client->dev,
			"[touch]%s:power off failed",
			__func__);

	mutex_unlock(&ts->lock);

	ts->is_sleep = 1;
	dev_info(&ts->client->dev, "[touch]enable touch lock.\n");
	return;
...
static void raydium_register_notifier(struct raydium_ts_data *raydium_ts)
{
    ...
    if(fb_register_client(&raydium_ts->fb_notif)){
		printk(KERN_ERR "[touch]register notifier failed\n");
	}
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	raydium_ts = devm_kzalloc(&client->dev,
...
*/
	...
	if (!ret && raydium_ts->ts_pinctrl) {
		...
		if (ret < 0)
			dev_err(&client->dev,
				"[touch]failed to set pin to active state\n");
...
#elif defined(CONFIG_HAS_EARLYSUSPEND)
	raydium_ts->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1; /*Early-suspend level*/
	...
	dev_err(&client->dev,
		"[touch]client irq : %d, pdata flags : %d\n",
		client->irq,
		pdata->irqflags);/*108*/

	ret = request_threaded_irq(gpio_to_irq(raydium_ts->pdata->irq_gpio),
				   NULL,
	...
	dev_err(&client->dev,
		"[touch]Raydium Touch driver version :0x%04X\n",
		RAYDIUM_VER);
