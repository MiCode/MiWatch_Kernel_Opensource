		...
		char *p_i8_buf)
{
	...
	do {
		...
		i32_ret = raydium_i2c_pda2_read(client, RAYDIUM_PDA2_HOST_CMD_ADDR,
					    u8_rbuffer, 1);
		if (i32_ret < 0)
			goto exit_i2c_error;
		...
		char *p_i8_buf)
{
	...
	mutex_lock(&ts->lock);
	i32_ret = raydium_i2c_pda_read(client, g_u32_addr, u8_rbuffer, g_u32_length);
	mutex_unlock(&ts->lock);
	if (i32_ret < 0)
		return i32_ret;

	sprintf(p_i8_buf, "0x%08X : 0x%02X%02X%02X%02X\n", (unsigned int)g_u32_addr,
		u8_rbuffer[3], u8_rbuffer[2], u8_rbuffer[1], u8_rbuffer[0]);
	u16_len = strlen(p_i8_buf);
				      ...
				      char *p_i8_buf)
{
	...
	if (g_u8_i2c_mode == PDA2_MODE) {
		...
		*(unsigned long *)u8_rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24)
					    | ((RAD_CHK_I2C_CMD &
						(~MASK_8BIT)) >> 8);

		i32_ret = raydium_i2c_pda2_write(client, RAYDIUM_PDA2_PDA_CFG_ADDR,
					     u8_rbuffer, 4);
		if (i32_ret < 0)
			goto exit_i2c_error;
	...
	sprintf(p_i8_buf, "[touch]RAD Touch check i2c: %02X%02X\n",
				     ...
				     char *p_i8_buf)
{
	...
	sprintf(p_i8_buf, "Raydium HW Reset : %d\n", i32_ret);
	pr_info("%s\n", p_i8_buf);
					...
					char *p_i8_buf)
{
	...
	sprintf(p_i8_buf, "[touch] palm_status : %d\n",
		...
		char *p_i8_buf)
{
	...
	if (i32_ret < 0)
		goto exit_i2c_error;
	sprintf(p_i8_buf, "RAD Touch FW Ver : %02X%02X%02X%02X\n",
		u8_rbuffer[0], u8_rbuffer[1], u8_rbuffer[3], u8_rbuffer[2]);

	fw_version = (u8_rbuffer[0] << 24) | (u8_rbuffer[1] << 16) | (u8_rbuffer[3] << 8)
		     | u8_rbuffer[2];
		...
		char *p_i8_buf)
{
	...
	if (i32_ret < 0)
		goto exit_i2c_error;
	sprintf(p_i8_buf, "RAD Touch Panel Version : %02X%02X%02X%02X%02X%02X\n",
		u8_rbuffer[0], u8_rbuffer[1], u8_rbuffer[2], u8_rbuffer[3], u8_rbuffer[4],
		u8_rbuffer[5]);
	mutex_unlock(&ts->lock);
	raydium_irq_control(ts, ENABLE);
...
exit_upgrade:
	...
	sprintf(p_i8_buf, "FW Upgrade result : %d\n", i32_result);
	u16_len = strlen(p_i8_buf);
	return u16_len + 1;
}
		...
		const char *p_i8_buf, size_t count)
{
	...
	strncpy(temp_buf, p_i8_buf, count);
				 ...
				 char *p_i8_buf)
{
	...
	sprintf(p_i8_buf, "%d", g_u8_raydium_flag);
		...
		char *p_i8_buf)
{
	...
	while (u32_offset < g_u32_raw_data_len) {
		...
		*(unsigned int *)u8_rbuffer = (RAYDIUM_I2C_PDA_MODE_ENABLE << 24)
		...
		const char *p_i8_buf, size_t count)
{
	...
	if (temp_buf && u32_data_count) {
		...
		while (u32_data_count) {
			token = strsep(&temp_buf, delim);
			i32_ret = kstrtou8(token, 16, &u8_w_data[u32_data_index++]);
			if (i32_ret < 0)
				goto exit_error;
			u32_data_count--;
		}
		...
		char *p_i8_buf)
{
	...
	mutex_lock(&ts->lock);
	i32_ret = raydium_i2c_pda2_read(client, g_u8_addr, u8_rbuffer, g_u32_length);
	mutex_unlock(&ts->lock);
	if (i32_ret < 0)
		return i32_ret;

	sprintf(p_i8_buf, "0x%04X : 0x%02X%02X%02X%02X\n", g_u8_addr,
		u8_rbuffer[3], u8_rbuffer[2], u8_rbuffer[1], u8_rbuffer[0]);
	u16_len = strlen(p_i8_buf);
					...
					const char *p_i8_buf, size_t count)
{
	...
	static unsigned char u8_type = 0;
	...
	if (count == 20) { /*check FW type*/
		...
		if (u8_cmd == RAD_CMD_UPDATE_BIN) { /*check FW length*/
			...
			if ((ts->id & 0x1000) == 0x1000)
				memcpy((g_rad_boot_image + RAD_BOOT_1X_SIZE), g_rad_init_image, 0x1FC);
			else if (((ts->id & 0x2000) == 0x2000) && (u8_type == RAYDIUM_TEST_FW))
				memcpy((g_rad_testfw_image + RAD_FW_2X_SIZE), g_rad_testpara_image, RAD_PARA_2X_SIZE);
