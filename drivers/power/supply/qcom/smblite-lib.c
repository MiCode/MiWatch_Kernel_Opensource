...
#define DEMO_MODE_BATT_LEVEL_LOW 40
#define DEMO_MODE_BATT_LEVEL_HIGH 85

int smblite_lib_get_prop_batt_capacity_fg(struct smb_charger *chg,
				  union power_supply_propval *val)
{
	int rc = -EINVAL;
	int raw_soc;
				  ...
				  union power_supply_propval *val)
{
	...
	rc = smblite_lib_get_prop_from_bms(chg, SMB5_QG_VOLTAGE_NOW,
							&voltage_now);
	if (rc < 0) {
		smblite_lib_err(chg, "Couldn't get now voltage rc=%d\n", rc);
	}
	rc = smblite_lib_get_prop_from_bms(chg, SMB5_QG_CAPACITY,
                                                &raw_soc);
	if ((rc < 0) && (rc != -EAGAIN))
		smblite_lib_err(chg, "Couldn't get capacity prop rc=%d\n", rc);
	if ((voltage_now > CAPACITY25_VOLTAGE) && (rc == -EAGAIN)){
		val->intval = 25;
		smblite_lib_err(chg, "voltage_now %d\n", voltage_now);
		return 0;
	}
	rc = smblite_lib_get_prop_batt_status(chg, &pval);
	if (rc < 0){
		smblite_lib_err(chg, "Couldn't get batt status rc=%d", rc);
	}
	rc = smblite_lib_get_prop_usb_present(chg, &usb_present);
	if (rc < 0){
		smblite_lib_err(chg, "Couldn't get usb present status rc=%d", rc);
	}

	if(raw_soc < 0 || raw_soc > 1000){
		/*No valid data obtain from slate, set fake soc to avoid abnormal shutdown*/
		pr_err("Invalid raw_soc from qbg : %d",raw_soc);
		if(voltage_now > CUTOFF_VOLTAGE) {
			pr_err("voltage_now = %d, set soc to 1", voltage_now);
			val->intval = 1;
		} else {
			pr_err("voltage_now = %d, set soc to 0", voltage_now);
			val->intval = 0;
		}
		return 0;
	}

	/* while ture SOC = 95%, report UI SOC to 100% */
	fg_soc = raw_soc / 10;
	val->intval = raw_soc * 100 / 95 / 10;

	if(val->intval > 100)
		val->intval = 100;
	if ((pval.intval == POWER_SUPPLY_STATUS_CHARGING) && last_soc) {
		if (last_soc > val->intval) {
			smblite_lib_err(chg, "battery monitor soc drop use last soc=%d soc=%d", last_soc, val->intval);
			val->intval = last_soc;
		}
		if (val->intval - last_soc > SOC_DIFFERENCE) {
			smblite_lib_err(chg, "Battery jump use last soc + 1,last soc=%d soc=%d", last_soc, val->intval);
			val->intval = last_soc + 1;
		}
	}
	if ((pval.intval != POWER_SUPPLY_STATUS_CHARGING) && last_soc) {
		if (last_soc < val->intval) {
			smblite_lib_err(chg, "battery monitor soc rise use last soc=%d soc=%d", last_soc, val->intval);
			val->intval = last_soc;
		}
	}
	/*Add for shutdown logic*/
	if ((val->intval == 0) && usb_present.intval) {
		if ((pval.intval == POWER_SUPPLY_STATUS_DISCHARGING) || (pval.intval == POWER_SUPPLY_STATUS_UNKNOWN) ) {
			if (voltage_now > SOC0_THRESHOLD) {
				//while discharging, if Vbatt > 3.45V do not report 0 soc
				pr_info("Battery is discharging with usb on, but vbatt = %d > %d, set battery soc to 1",
						voltage_now, SOC0_THRESHOLD);
				val->intval = 1;
			}
		} else {
			//while charging, do not report 0 soc
			pr_info("Battery is charging do not report 0 soc");
			val->intval = 1;
		}
	}
	if(val->intval == 0)
		pr_err("Now SoC is 0, Vbat=%d, battery status=%d, usb present=%d", voltage_now, pval.intval, usb_present.intval);

	last_soc = val->intval;

	if (chg->is_demo_mode) {
		if (val->intval > DEMO_MODE_BATT_LEVEL_HIGH) {
			vote(chg->chg_disable_votable, USER_VOTER, true, 0);
			pr_info("[demo mode] capacity: %d > 85, disable charging\n",
					val->intval);
		} else if (val->intval < DEMO_MODE_BATT_LEVEL_LOW) {
			vote(chg->chg_disable_votable, USER_VOTER, false, 0);
			pr_info("[demo mode] capacity: %d < 40, enable charging\n",
					val->intval);
		}
	}
...
int smblite_lib_set_prop_is_demo_mode(struct smb_charger *chg,
				const union power_supply_propval *val)
{
	if (val->intval)
		chg->is_demo_mode = 1;
		...
		chg->is_demo_mode = 0;
		vote(chg->chg_disable_votable, USER_VOTER, false, 0);
		pr_info("Exit demo mode, enable charging\n");
	}
...
int smblite_lib_get_prop_is_demo_mode(struct smb_charger *chg, int *is_demo_mode)
{
	*is_demo_mode = chg->is_demo_mode;
...
static void smblite_lib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		/*Store demo mode demand*/
		if (chg->is_demo_mode) {
			if(batt_capacity >= DEMO_MODE_BATT_LEVEL_LOW) {
				vote(chg->chg_disable_votable, USER_VOTER, true, 0);
				pr_info("[demo mode]capacity: %d >= 40, disable charging\n",
						batt_capacity);
			}
		}
	} else {
