...
static void slate_irq_tasklet_hndlr_l(void)
{
	...
	if (slav_status_auto_clear_reg & OK_TO_SLEEP_CLEARED) {
		SLATECOM_INFO("SLAVE_STATUS_READY = 0x%08X, OK_TO_SLEEP_CLEARED = 0x%08X, ISR: 0x%08X, FIFO: 0x%08X, SIZE: 0x%08X", (slave_status_reg & SLAVE_STATUS_READY),(OK_TO_SLEEP_CLEARED & slav_status_auto_clear_reg), glink_isr_reg, fifo_fill_reg, fifo_size_reg);
...
static int slatecom_pm_suspend(struct device *dev)
{
	...
	if (((g_slave_status_auto_clear_reg & OK_TO_SLEEP_CLEARED) &&
			(g_slav_status_reg & SLAVE_STATUS_READY))
                         || atomic_read(&slate_is_spi_active)) {
