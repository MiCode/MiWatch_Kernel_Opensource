...
#define RSB_CALI_FILE  "/mnt/vendor/persist/sensors/rsb_calic"
struct event {
	uint8_t sub_id;
	int16_t evnt_data;
	uint64_t evnt_tm;
...
int read_rsb_calivalue_from_file(int *cali_re)
{
	struct file *fp = NULL;
	int f_size = 10;
	char *buf = NULL;
	...
	fp = filp_open(RSB_CALI_FILE, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		printk("rsb cali file open %s failed!", RSB_CALI_FILE);
		return -EINVAL;
	}

	buf = kzalloc(f_size + 1, GFP_ATOMIC);
	if (!buf) {
		printk("rsb malloc mem %d failed!", f_size);
		filp_close(fp, NULL);
		return -EINVAL;
	}
	...
	printk("rsb buf:%s int_cali_re: %d", buf, int_cali_re);
...
static int show_rsbvalue(struct device *dev, struct device_attribute *attr,
			char *buff)
{
	struct seb_priv *pdev = dev_get_drvdata(dev);

	return sprintf(buff,"%d\n", pdev->rsb_value );
}
static ssize_t store_rsbvalue(struct device *pdev, struct device_attribute *attr,
		const char *buff, size_t count)
{
	struct seb_priv *dev = dev_get_drvdata(pdev);
	int value_stage = 0;

	if (kstrtoint(buff, 10, &value_stage) < 0)
		return -ENOMEM;
	mutex_lock(&dev->seb_state_mutex);
	dev->rsb_value = value_stage;
	printk("store rsb value :%d", dev->rsb_value);
...
void seb_send_input(struct event *evnt)
{
	...
	if (evnt->sub_id == GMI_SLATE_EVENT_RSB) {
		...
		printk("rsb_value:%d",evnt->evnt_data);
		input_sync(dev->input);
		dev->rsb_value += evnt->evnt_data;
		printk("rsb_value:%d",dev->rsb_value);
...
static int seb_probe(struct platform_device *pdev)
{
	...
	if (rc)
		pr_err("Not able to create the file rsbvalue\n");
