...
struct slatersb_priv {
	...
	struct work_struct rsb_calibration_init_work;
...
static int slatersb_enable(struct slatersb_priv *dev, bool enable)
{
	...
	pr_err("req.data = %d, req.cmd_id = %d\n", req.data, req.cmd_id);
...
void slatersb_notify_glink_channel_state(bool state)
{
	...
	pr_err("%s: RSB-CTRL channel state: %d\n", __func__, state);
	dev->rsb_rpmsg = state;
}
...
static void slatersb_slateup_work(struct work_struct *work)
{
	...
	mutex_lock(&dev->rsb_state_mutex);
		...
		pr_err("slatersb-rpmsg is probed\n");
...
static void slatersb_enable_rsb(struct work_struct *work)
{
	...
	{
		cali_init_flag = 1;
		queue_work(dev->slatersb_wq, &dev->rsb_calibration_init_work);
	}
...
static void slatersb_calibration_init(struct work_struct *work)
{
	int rc = 0;
	int rsbcali = 0;
	struct slatersb_msg req = {0};
	...
	rc = read_rsb_calivalue_from_file(&rsbcali);
	if (rc)
	{
		pr_err("Not able to read rsb cali value\n");
	}
	if(rsbcali == 0)
		goto unlock;

	pr_err("read rsb cali value :%d\n", rsbcali);
...
static int slatersb_init(struct slatersb_priv *dev)
{
	...
	INIT_WORK(&dev->rsb_calibration_init_work, slatersb_calibration_init);
