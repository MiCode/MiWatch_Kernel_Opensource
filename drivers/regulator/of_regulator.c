...
extern int board_id_get_hw_version(void);
					...
					const struct regulator_desc *desc)
{
	...
	/*pr_err("%pOFn: Current status is %d constraints->name: %s\n", np, constraints->always_on, constraints->name);*/

	lrs_board_id = board_id_get_hw_version();
	/*printk("lrs_board_id:%d",lrs_board_id);*/
	if(lrs_board_id==101||lrs_board_id==110) {
		/*printk("lrs enter P2.0 Power on Plan\n");*/
		if (!strcmp(constraints->name, "pm5100_l12")||!strcmp(constraints->name, "pm5100_l25")) {
			pr_err("%s: Current status is %d\n", constraints->name, constraints->always_on);
			constraints->always_on = 0;
		}
	}
